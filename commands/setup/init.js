...
`;
export const defaultCommand = true;
...
export default async (_cli, _config) => {
  ...
  log.plain('Ahoy!\n');
  log.plain(
    'This wizard will guide you through creating a config file for setting up Insights frontend applications.\n'
  );
  ...
  const basePath = await inputPrompt(
    ...
    currentConfig.basePath || process.env.HOME,
    {
      validate: (
        ...
        ? 'Path can not be the home directory'
        : true)
    }
  );
