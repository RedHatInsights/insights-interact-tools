...
import { buildTable, log } from './helpers.js';
...
const globalFlags = {
  config: {
    ...
    alias: 'c',
    description:
      'Define a custom config (defaults to ~/.insights-interact.yml)'
  },
  debug: {
    type: 'boolean',
    alias: 'd',
...
const importCommandModule = async (
  commandsPath,
  commandDirectory,
  moduleFile
) => {
...
const importCommandModules = async () => {
  ...
  for (const commandDirectory of commandDirectories) {
    ...
    const moduleFiles = readdirSync(path.join(commandsPath, commandDirectory), {
      withFileTypes: true
    }).filter(({ name }) => name !== 'index.js' && name !== '.DS_Store');

    for (const { name: moduleFile } of moduleFiles) {
      const [moduleName, module] = await importCommandModule(
        commandsPath,
        commandDirectory,
        moduleFile
      );
...
const getCommandModuleFlags = (commandModules) =>
  Object.entries(commandModules).flatMap(
    ([_commandCategory, commandCategoryModules]) => {
      return Object.entries(commandCategoryModules).map(([_command, module]) =>
        module.flags
      );
    }
...
const renderFlags = (flags) => {
  const rows = Object.entries(flags).map(([flag, options]) => [
    ...
    Array.isArray(options?.alias)
      ? options.alias.map((alias) => '-' + alias).join(', ')
      : '-' + options.alias,
    options.type,
    options.description || ''
  ]);
...
const getCommandModulesHelp = (commandModules) => {
  return Object.entries(commandModules).flatMap(
    ([commandModuleCategory, commandCategoryModules]) => {
      return [
        ...Object.entries(commandCategoryModules).flatMap(
          ([commandName, module]) => {
            const flags = module.flags
              ? [
                  chalk.bold('    Flags'),
                  renderFlags(module.flags).split('\n').join('\n      ')
                ]
              : [];
...
const getSubCommandModuleCommand = (commandModules, command, subCommand) => {
  const commandModule = commandModules[command];
  const subCommandModule = commandModule[subCommand];
  if (!subCommandModule) {
    const defaultCommand = Object.values(commandModule).find((
      commandModule
    ) => commandModule.defaultCommand);

    return defaultCommand;
  } else {
    return subCommandModule;
  }
};
...
const run = async (cli, commandModules) => {
  ...
  if ((!command && !subCommand) || cli.flags.help) {
    ...
    const commandModule = getSubCommandModuleCommand(
      commandModules,
      command,
      subCommand
    );
